    ...
    if (googleToken != undefined && googleToken.renew != undefined && new Date().getTime() / 1000 < parseFloat(googleToken.renew)) {
      ...
      gapi.auth.authorize(config, function (data) {
        if (gapi.auth.getToken()) {
          ...
          reject({code: 0, message: "Authorize error"});
        }
      }, function (reason) {
        console.log("NO TOKEN" + reason);
        reject({code: 0, message: "No token"});
      });
    }
  })
}
...
function fileList(success, error, args) {
  ...
  gapi.client.load('drive', 'v2').then(function () {
    ...
    error(angular.toJson({code: 0, message: "client load drive"}))
  });
...
function renameFile(success, error, args) {
  ...
  request.execute(function (resp) {
    if (resp.error) {
      console.log("error" + angular.toJson(resp.data));
      error({code: 0, message: "renameFile"});
    } else {
      success();
    }
  });
...
function createFile(success, error, args) {
  ...
  request.error = function (evt) {
    error({code: 0, message: "Error createFile"});
  };
...
function downloadFile(success, error, args) {
  ...
  xhr.onerror = function (evt) {
    error({code: 0, message: "Error download"});
  };
  xhr.send();
...
var GoogleDrive = {
  ...
  downloadFile: function (win, fail, args) {
    login().then(
      function () {
        downloadFile(win, fail, args)
      }
    );
  },

  createFile: function (win, fail, args) {
    login().then(
      function () {
        createFile(win, fail, args)
      }
    );
  },

  renameFile: function (win, fail, args) {
    login().then(
      function () {
        renameFile(win, fail, args)
      }
    );
  },
  fileList: function (win, fail, args) {
    login().then(
      function () {
        fileList(win, fail, args);
      }
    );
  }
